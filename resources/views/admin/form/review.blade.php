                ...
                <div class="col-lg-offset-1 col-lg-10">
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            <h3>講者: {{$reviewData['speaker']->talker_name}}</h3>
                            <h3>主題: {{$reviewData['event'][0]->topic}}</h3>
                        </div>

                        <div class="panel-body">
                            <form action='/event' id="reviewForm" method="POST" class="form-horizontal">
                                {{ csrf_field() }}
                                    ...
                                    <div class="table-responsive sg-rating-scale">
                                        <table class="table table-striped">
                                            ...
                                            @for ($line = 0; $line <= 5; $line++)
                                                <tr>
                                                    <td>{{$reviewData['score'][$line]["title"]}}</td>
                                                    @for ($i = 5; $i > 0; $i--)
                                                        <td>
                                                            <div class="radio" style="text-align: left;">
                                                                <label class="sg-cell sg-cell-{{$i}} sg-cell-data">
                                                                    <input name="{{$reviewData['score'][$line]["name"]}}" id="{{$reviewData['score'][$line]["name"]}}-{{$line}}-{{$i}}" type="radio" class="sg-input sg-input-radio"
                                                                           value="{{ $i }}"
                                                                           @if (old($reviewData['score'][$line]["name"]) == $i) checked @endif>
                                                                    <label for="{{$reviewData['score'][$line]["name"]}}-{{$line}}-{{$i}}"></label>
                                                                </label>
                                                            </div>
                                                        </td>
                                                    @endfor
